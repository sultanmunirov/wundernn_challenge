...
class _LSTMFullPredictor(nn.Module):
    def __init__(self, input_dim, hidden_dim=256, num_layers=2, dropout=0.1):
        ...
        self.layer_norm_after_lstm = nn.LayerNorm(hidden_dim)
    ...
    def _apply_chrono_init(self, hidden_size, T_max):
        """
        Инициализирует смещения Update Gate в LSTM логарифмически.
        """
        with torch.no_grad():
    ...
    def forward(self, x, hidden=None):
        ...
        out = self.layer_norm_after_lstm(out)
        preds = self.head(out)
        return preds, hidden
    ...
    def step(self, x_t, hidden=None):
        ...
        last = self.layer_norm_after_lstm(last)
        pred = self.head(last)
        return pred, hidden
...
class LSTMFullModel:
    def __init__(
        ...
        self.models = [
            _LSTMFullPredictor(self.input_dim, hidden_dim, num_layers, dropout).to(
                self.device
            )
        ...
        self.model_name = f"lstm_chrono_full_h{hidden_dim}_lr{lr_str}_wd{wd_str}"
    ...
    def _train_model(self, train_seqs, val_seqs=None):
        set_seed()
        print(
            f"Training {self.model_name} with Multi-Augmentation (5x Data + Noise)..."
        )
        df = pd.read_parquet(
        ...
        print(
            f"Dataset Min Var: {np.min(seq_vars):.6f}, Max Var: {np.max(seq_vars):.6f}"
        )
        ...
        print(
            f"Generating {NUM_AUGMENTATIONS}x Augmented Data with Random Scaling + Noise..."
        )
        ...
        print(
            f"Total Train Samples : {len(train_pairs)} (Original: {len(train_pairs) - len(aug_train_pairs)})"
        )
        print("-------------------------------\n")

        val_pairs_full = val_pairs
        val_pairs_low, val_pairs_high = [], []

        var_dict = {}
        for sid in val_seqs:
            states = seqs.get_group(sid).sort_values("step_in_seq").iloc[:, 3:].values
            var_dict[sid] = np.var(states, axis=0).mean() if len(states) > 0 else 0.0
        sorted_val = sorted(var_dict.items(), key=lambda x: x[1])
        half = len(sorted_val) // 2
        low_ids = [s for s, _ in sorted_val[:half]]
        high_ids = [s for s, _ in sorted_val[half:]]

        for sid in low_ids:
            st, ft, npred = seq_arrays(sid)
            if len(st) >= 2:
                val_pairs_low.append((st, ft, npred))
        for sid in high_ids:
            st, ft, npred = seq_arrays(sid)
            if len(st) >= 2:
                val_pairs_high.append((st, ft, npred))
        ...
        full_Y_low = self._collect_targets(val_loader_low)
        full_Y_high = self._collect_targets(val_loader_high)
        ...
        for boost_idx in range(self.num_boosts):
            ...
            top_k_queue = TopKModelQueue(k=5)
            ...
            for epoch in range(self.max_epochs[boost_idx]):
                ...
                val_metrics = []
                loaders = [
                    (val_loader_full, full_Y_full, "full"),
                    (val_loader_low, full_Y_low, "low"),
                    (val_loader_high, full_Y_high, "high"),
                ]
                stage_r2s, val_losses = [], []

                for loader, _, name in loaders:
                    if loader is None:
                        continue
                    vl, sr2 = self._compute_stage_metrics(
                        loader, model, prev_models, crit, self.device
                    )
                    val_losses.append(vl)
                    stage_r2s.append(sr2)

                val_loss = np.mean(val_losses) if val_losses else 0.0
                stage_r2_full = stage_r2s[0] if stage_r2s else 0.0

                dt = time.time() - t0
                print(
                    f"Boost {boost_idx+1} Epoch {epoch+1}: TrainLoss={train_loss:.4f} ValLoss={val_loss:.4f} GlobalR2_full={stage_r2_full:.4f} ({dt:.2f}s)"
                )

                raw_state_dict = {
                    k: v.cpu().clone() for k, v in model.state_dict().items()
                }
                top_k_queue.update(stage_r2_full, raw_state_dict)
                ...
                if avg_state_dict is not None:
                    ...
                    print(f" [SWA] Avg Model Full Global R2: {avg_r2_full:.4f}")
            ...
            if best_stage_state is not None:
                ...
                model.eval()
                print(
                    f"Loaded best for boost {boost_idx + 1}: Global R2 full={best_stage_r2:.4f}"
                )
            ...
            if cum_r2_full > self.best_val_r2:
                ...
                print(f" -> New global best cum R2 (full val): {cum_r2_full:.4f}")
            else:
                patience_counter += 1
                if patience_counter >= self.patience:
                    print(f"Global early stop after boost {boost_idx + 1}")
                    break

        if os.path.exists(self.best_models_path):
            states = torch.load(self.best_models_path, map_location=self.device)
    ...
    def _collect_preds(self, model, loader):
        ...
        model.eval()
        with torch.inference_mode():
            for Xpad, _, pmask, L in loader:
                if Xpad.numel() == 0:
                    continue
                Xpad = Xpad.to(self.device, non_blocking=True)
                mask = self._get_mask(Xpad.size(0), Xpad.size(1), L, device=self.device)
                final_mask = mask & pmask.to(self.device)
                P, _ = model(Xpad)
                preds_list.append(P[final_mask].cpu().numpy())
        return (
    ...
    def _collect_cum_preds(self, models_slice, loader):
        cum_preds = np.zeros((0, BASE_OUT_DIM))
        for model in models_slice:
            stage_preds = self._collect_preds(model, loader)
            if len(cum_preds) == 0:
                cum_preds = stage_preds
            else:
                cum_preds += stage_preds
        return cum_preds
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(seq_ids)):
        ...
        model.best_models_path = f"models/{model.model_name}.pt"
        model.per_boost_paths = [
            f"models/{model.model_name}_boost{i}.pt" for i in range(model.num_boosts)
        ]

        model._train_model(
            [seq_ids[i] for i in train_idx], [seq_ids[i] for i in val_idx]
        )
        model.save_model(model.best_models_path)
        fold_results.append(model.best_val_r2)
        print(f"Fold {fold+1} Best Val R2: {model.best_val_r2:.4f}")

    print(f"\nMean R2: {np.mean(fold_results):.4f} +/- {np.std(fold_results):.4f}")
