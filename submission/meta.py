...
class PredictionModel:
    def __init__(self, lstm_epochs=50, gru_epochs=50, bilstm_epochs=10):
        self.input_dim = 32
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.lstm = LSTMFullModel(num_epochs=lstm_epochs)
        self.gru_full = GRUFullModel(num_epochs=gru_epochs)
        self.bilstm = BiLSTMModel(num_epochs=bilstm_epochs)

        lstm_path = "submission/models/lstm_full.pt"
        self.lstm.load_model(lstm_path)

        self._reset_state()
        gru_full_path = "submission/models/gru_full.pt"
        self.gru_full.load_model(gru_full_path)

        bilstm_path = "submission/models/bilstm.pt"
        self.bilstm.load_model(bilstm_path)

        self.weights = torch.tensor(np.load("submission/models/meta_weights.npy"))
    ...
    def _optimize_weights(self, val_seqs):
        ...
        paths = {
            ...
            "gru_full": f"{oof_dir}/oof_gru_full.npy",
        ...
        preds_gf = torch.from_numpy(preds["gru_full"]).float().to(self.device)
        ...
        p_g = preds["gru_full"][:n_samples].reshape(n_seq, self.seq_len, 32)
        ...
        r2_per_feat = [
            r2_score(flat_targets[:, i], simulated_preds[:, i]) for i in range(32)
        ]
        mean_r2 = np.mean(r2_per_feat)
    ...
    def _reset_state(self):
        for m in (self.lstm, self.gru_full, self.bilstm):
            m.reset_state()
        self.current_seq_ix = None
        self.adaptive_weights_np = None
        self.last_raw_preds = None
    ...
    def predict(self, data_point) -> np.ndarray | None:
        ...
        p_g = self.gru_full.predict(data_point)
